...
package org.heigit.ors.api.requests.pt;
...
import org.heigit.ors.api.requests.routing.RouteRequestAlternativeRoutes;
...
public class PublicTransportRequest extends APIRequest {
...
@ApiModelProperty(name = PARAM_MAXIMUM_SPEED, value = "The maximum speed specified by user." +
    ...
    @JsonCreator
    public PublicTransportRequest(@JsonProperty(value = PARAM_COORDINATES, required = true) List<List<Double>> coordinates) {
        this.coordinates = coordinates;
    }

    public PublicTransportRequest(Double[][] coordinates) throws ParameterValueException {
    ...
    public PublicTransportRequest(Coordinate start, Coordinate end) throws ParameterValueException {
        if(start == null) {
    ...
    public RoutingRequest convertRouteRequest() throws StatusCodeException {
        ...
        if (this.hasSimplifyGeometry()) {
            ...
            if (this.hasExtraInfo() && simplifyGeometry) {
                throw new IncompatibleParameterException(RoutingErrorCodes.INCOMPATIBLE_PARAMETERS, PublicTransportRequest.PARAM_SIMPLIFY_GEOMETRY, "true", PublicTransportRequest.PARAM_EXTRA_INFO, "*");
            }
        }
        ...
        try {
            ...
            throw new ParameterValueException(RoutingErrorCodes.INVALID_PARAMETER_VALUE, PublicTransportRequest.PARAM_PROFILE);
        }
        ...
        if (this.hasAlternativeRoutes()) {
            if (coordinates.size() > 2) {
                throw new IncompatibleParameterException(RoutingErrorCodes.INCOMPATIBLE_PARAMETERS, PublicTransportRequest.PARAM_ALTERNATIVE_ROUTES, "(number of waypoints > 2)");
            }
            if (alternativeRoutes.hasTargetCount()) {
                ...
                if (countLimit > 0 && alternativeRoutes.getTargetCount() > countLimit) {
                    throw new ParameterValueException(RoutingErrorCodes.INVALID_PARAMETER_VALUE, PublicTransportRequest.PARAM_ALTERNATIVE_ROUTES, Integer.toString(alternativeRoutes.getTargetCount()), "The target alternative routes count has to be equal to or less than " + paramMaxAlternativeRoutesCount);
                }
            }
        ...
        if (this.hasDeparture() && this.hasArrival())
            throw new IncompatibleParameterException(RoutingErrorCodes.INCOMPATIBLE_PARAMETERS, PublicTransportRequest.PARAM_DEPARTURE, PublicTransportRequest.PARAM_ARRIVAL);
    ...
    private List<Integer> processSkipSegments() throws ParameterOutOfRangeException, ParameterValueException, EmptyElementException {
        for (Integer skipSegment : skipSegments) {
            if (skipSegment >= coordinates.size()) {
                throw new ParameterOutOfRangeException(RoutingErrorCodes.INVALID_PARAMETER_VALUE, PublicTransportRequest.PARAM_SKIP_SEGMENTS, skipSegment.toString(), String.valueOf(coordinates.size() - 1));
            }
            if (skipSegment <= 0) {
                throw new ParameterValueException(RoutingErrorCodes.INVALID_PARAMETER_VALUE, PublicTransportRequest.PARAM_SKIP_SEGMENTS, skipSegments.toString(), "The individual skip_segments values have to be greater than 0.");
            }

        }
        if (skipSegments.size() > coordinates.size() - 1) {
            throw new ParameterValueException(RoutingErrorCodes.INVALID_PARAMETER_VALUE, PublicTransportRequest.PARAM_SKIP_SEGMENTS, skipSegments.toString(), "The amount of segments to skip shouldn't be more than segments in the coordinates.");
        }
        if (skipSegments.isEmpty()) {
            throw new EmptyElementException(RoutingErrorCodes.EMPTY_ELEMENT, PublicTransportRequest.PARAM_SKIP_SEGMENTS);
        }
        return skipSegments;
    }
    ...
    private boolean convertIncludeGeometry() throws IncompatibleParameterException {
        if (!includeGeometry && responseType != APIEnums.RouteResponseType.JSON) {
            throw new IncompatibleParameterException(RoutingErrorCodes.INVALID_PARAMETER_VALUE,
                    PublicTransportRequest.PARAM_GEOMETRY, "false",
                    PublicTransportRequest.PARAM_FORMAT, APIEnums.RouteResponseType.GEOJSON + "/" + APIEnums.RouteResponseType.GPX);
        }
        return includeGeometry;
    }
    ...
    private String convertGeometryFormat(APIEnums.RouteResponseType responseType) throws ParameterValueException {
        switch (responseType) {
            ...
            case GPX:
                return "gpx";
            default:
                throw new ParameterValueException(RoutingErrorCodes.INVALID_PARAMETER_VALUE, PublicTransportRequest.PARAM_FORMAT);
        }
    }
    ...
    private Coordinate[] convertCoordinates(List<List<Double>> coordinates, boolean allowSingleCoordinate) throws ParameterValueException {
        if (!allowSingleCoordinate && coordinates.size() < 2)
            throw new ParameterValueException(RoutingErrorCodes.INVALID_PARAMETER_VALUE, PublicTransportRequest.PARAM_COORDINATES);

        if (allowSingleCoordinate && coordinates.size() > 1)
            throw new ParameterValueException(
                    RoutingErrorCodes.INVALID_PARAMETER_VALUE,
                    PublicTransportRequest.PARAM_COORDINATES,
    ...
    private WayPointBearing[] convertBearings(Double[][] bearingsIn, int coordinatesLength) throws ParameterValueException {
        ...
        if (bearingsIn.length != coordinatesLength && bearingsIn.length != coordinatesLength - 1)
            throw new ParameterValueException(RoutingErrorCodes.INVALID_PARAMETER_VALUE, PublicTransportRequest.PARAM_BEARINGS, Arrays.toString(bearingsIn), "The number of bearings must be equal to the number of waypoints on the route.");
    ...
    private double[] convertMaxRadii(Double[] radiiIn, int coordinatesLength, int profileType) throws ParameterValueException {
        if (radiiIn != null) {
            ...
            if (radiiIn.length != coordinatesLength)
                throw new ParameterValueException(RoutingErrorCodes.INVALID_PARAMETER_VALUE, PublicTransportRequest.PARAM_RADII, Arrays.toString(radiiIn), "The number of specified radiuses must be one or equal to the number of specified waypoints.");
    ...
    private RouteInstructionsFormat convertInstructionsFormat(APIEnums.InstructionsFormat formatIn) throws UnknownParameterValueException {
        ...
        if (instrFormat == RouteInstructionsFormat.UNKNOWN)
            throw new UnknownParameterValueException(RoutingErrorCodes.INVALID_PARAMETER_VALUE, PublicTransportRequest.PARAM_INSTRUCTIONS_FORMAT, formatIn.toString());
    ...
    private int convertWeightingMethod(APIEnums.RoutePreference preferenceIn) throws UnknownParameterValueException {
        ...
        if (weightingMethod == WeightingMethod.UNKNOWN)
            throw new UnknownParameterValueException(RoutingErrorCodes.INVALID_PARAMETER_VALUE, PublicTransportRequest.PARAM_PREFERENCE, preferenceIn.toString());
    ...
    private boolean convertSetFlexibleMode(boolean useContractionHierarchies) throws ParameterValueException {
        if (useContractionHierarchies)
            throw new ParameterValueException(RoutingErrorCodes.INVALID_PARAMETER_FORMAT, PublicTransportRequest.PARAM_OPTIMIZED);
